...
import { Phone, Link, FileText, User, Upload, ArrowRight, ShieldCheck, DollarSign } from "lucide-react";
...
export default function InvestorDashboard() {
    ...
    fetch("/api/profile/get-profile", {
      ...
      .then(data => {
        if (data.success && data.profile) {
          ...
          setInvestmentFocus(p.investmentFocus || "");
          setPreferredStage(p.preferredStage || "");
          setTicketSize(p.ticketSize || "");
          setFinancialCapacity(p.financialCapacity || "");
          setInvestmentCurrency(p.investmentCurrency || "INR");
          setBio(p.bio || "");
          setPhoto(p.photo || "");
        }
      })
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phone || !investorType || !investmentFocus || !financialCapacity || !bio || !photo) {
    ...
    if (bio.length < 100) {
      setError("Bio/Mandate description must be at least 100 characters");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        updateUser({
          ...
          investorProfile: data.user.investorProfile,
        });
  ...
  const bioRemaining = 100 - bio.length;
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <div className="bg-slate-900/90 border border-slate-700/80 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl font-black tracking-tight mb-2 text-white">Investor Profile</h1>
            <p className="text-slate-300 text-xs font-medium">Outline your investment criteria to find matching startups</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-extrabold text-slate-200 uppercase tracking-wider">
                  Investment Focus <span className="text-rose-500">*</span>
                </label>
                <select
              ...
              {/* Preferred Stage */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-extrabold text-slate-200 uppercase tracking-wider">Preferred Stage</label>
                <select
                ...
                >
                  ...
                  <option value="Early Growth (Series A)">Early Growth (Series A)</option>
                  <option value="Growth Stage (Series B+)">Growth Stage (Series B+)</option>
                </select>
              </div>

              {/* Ticket Size */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-extrabold text-slate-200 uppercase tracking-wider">Typical Ticket Size</label>
                <select
                  value={ticketSize}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-extrabold text-slate-200 uppercase tracking-wider">
                  Financial Capacity <span className="text-rose-500">*</span>
                </label>
                <select
                  ...
                  className="w-full bg-slate-950/90 border border-slate-700/90 rounded-xl py-3 px-4 text-slate-200 font-semibold focus:outline-none focus:border-indigo-500 transition text-sm cursor-pointer shadow-inner"
                >
                  <option value="">Select Capability Tier</option>
                  <option value="Good">Good ($50k - $100k yearly allocation)</option>
                  <option value="Nice">Nice ($100k - $250k yearly allocation)</option>
                  <option value="Very Nice">Very Nice ($250k - $1M yearly allocation)</option>
                  <option value="Excellent">Excellent ($1M+ yearly allocation)</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-extrabold text-slate-200 uppercase tracking-wider flex items-center justify-between">
                <span>
                  Investment Mandate & Bio <span className="text-rose-500">*</span>
                </span>
                <span
                  className={`text-xs font-bold ${
                    bioRemaining <= 0 ? "text-emerald-400 font-extrabold" : "text-slate-400"
                  }`}
                >
                  {bioRemaining > 0 ? `${bioRemaining} more characters needed` : "Looks good!"}
                </span>
              </label>
              <div className="relative">
                ...
                <textarea
                  rows={4}
                  required
                  placeholder="Describe your background, prior investments, value-add besides capital, and what you look for in a founding team (Min. 100 characters)..."
                  value={bio}
            ...
            >
              {loading ? "Saving Mandate..." : "Save and Discover Startups"}
